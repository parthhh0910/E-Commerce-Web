...
const getProductImage = (product) => {
  ...
  return PRODUCT_IMAGES[product.name] || `https://loremflickr.com/600/400/${product.category ? encodeURIComponent(product.category.toLowerCase()) : 'gadget'}?lock=${product.id}`;
};
                ...
                <Link to={`/product/${id}`} style={{ textDecoration: "none", color: "inherit", display: "flex", flexDirection: "column", height: "100%" }}>
                  <div className="home-card__img-wrap">
                    <img
                      ...
                      onError={(e) => { e.target.onerror = null; e.target.src = `https://loremflickr.com/600/400/${product.category ? encodeURIComponent(product.category.toLowerCase()) : 'gadget'}?lock=${id}`; }}
                    />
                  </div>
